---
id: COMM-U1
title: TT&C 트랜스폰더 유닛 (설계~제작~RF시험)
status: TAKEN
parent: M-COMM
source: M-COMM
owner: COMM-LEAD-01
deliverable: examples/ksat8/deliverables/COMM/comm-u1-fm-package.md
after: -
track: COMM
started: 2026-08-27 03:47:14
finished: 2026-08-27 04:01:40
---

TT&C 트랜스폰더 1식(수신기·복조기·명령복호·TM변조·송신기·레인징 회로)의
설계 풀체인(DSN→ANL 2명→CHK→RVW 2명→RB→CM 요청)과 제작·RF시험까지의
전체 인수인계 사슬을 취합하는 Work. 자식 체인이 모두 끝나면 FM 패키지로
취합한다.
산출물: examples/ksat8/deliverables/COMM/comm-u1-fm-package.md
검증: 설계~제작~RF시험 전 체인 DONE, NCR 3건 이월(안테나/부품입고/CAL·FAC)
